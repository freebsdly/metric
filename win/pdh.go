...
type PDH_COUNTER_PATH_ELEMENTS struct {
	...
	ObjectName     string
	InstanceName   string
	iarentInstance uintptr
	instanceIndex  uintptr
	CounterName    string
}
...
// Union specialization for double values,
// used by PdhGetFormattedCounterArrayDouble()
type PDH_FMT_COUNTERVALUE_ITEM_DOUBLE struct {
	SzName   *uint16 // pointer to a string
	FmtValue PDH_FMT_COUNTERVALUE_DOUBLE
}

// Union specialization for 'large' values.
// used by PdhGetFormattedCounterArrayLarge()
...
// Union specialization for long values,
// used by PdhGetFormattedCounterArrayLong()
...
// Remove counter from Query
func PdhRemoveCounter(counter PDH_HCOUNTER) uint32 {
	ret, _, _ := pdh_RemoveCounter.Call(uintptr(counter))
	return uint32(ret)
}

// Adds the specified language-neutral counter to the query.
// See the PdhAddCounter function. This function only exists on
...
// Formats the given hCounter using a large int (int64).
// The result is set into the specialized union struct pValue.
// This function does not directly translate to a Windows counterpart due to union specialization tricks.
...
func PdhGetFormattedCounterArrayDouble(
	...
	lpdwBufferSize *uint32,
	lpdwBufferCount *uint32,
	itemBuffer *PDH_FMT_COUNTERVALUE_ITEM_DOUBLE,
) uint32 {
...
func PdhGetFormattedCounterArrayLarge(
	...
	lpdwBufferSize *uint32,
	lpdwBufferCount *uint32,
	itemBuffer *PDH_FMT_COUNTERVALUE_ITEM_LARGE,
) uint32 {
	ret, _, _ := pdh_GetFormattedCounterArrayW.Call(
		uintptr(hCounter),
		uintptr(PDH_FMT_LARGE),
...
func PdhGetFormattedCounterArrayLong(
	...
	lpdwBufferSize *uint32,
	lpdwBufferCount *uint32,
	itemBuffer *PDH_FMT_COUNTERVALUE_ITEM_LONG,
) uint32 {
	ret, _, _ := pdh_GetFormattedCounterArrayW.Call(
		uintptr(hCounter),
		uintptr(PDH_FMT_LONG),
...
// get all counters name with given objectname.
// you should call this function twice, the first time to get the required buffer size
//(set the buffers to NULL and the sizes to 0), and the second time to get the data.
//
func PdhEnumObjectItems(
	szDataSource uintptr,
	szMachineName uintptr,
	szObjectName string,
	mszCounterList *uint16,
	pcchCounterListLength *uint32,
	mszInstanceList *uint16,
	pcchInstanceListLength *uint32,
	dwDetailLevel uint32,
	dwFlags uint32,
) uint32 {
	otxt, _ := syscall.UTF16PtrFromString(szObjectName)
	ret, _, _ := pdh_EnumObjectItemsW.Call(
		szDataSource,
		szMachineName,
		uintptr(unsafe.Pointer(otxt)),
